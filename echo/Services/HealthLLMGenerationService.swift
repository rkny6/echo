...
/// Generates LLM companion messages for proactive health events.
actor HealthLLMGenerationService {
    ...
    /// Generate a companion message for a health event.
    /// - Returns: The generated content, or `nil` if the LLM call failed/timed
    ///   out/came back empty — callers should skip sending anything in that
    ///   case rather than falling back to a canned template.
    func generateMessage(
        ...
        do {
            ...
            guard !trimmed.isEmpty else {
                throw HealthLLMError.emptyResponse
            }
            await logger.log(
                "Health LLM generated message for \(event.type.rawValue)",
                level: .info
            )
            return trimmed
        } catch {
            await logger.log(
                "Health LLM failed for \(event.type.rawValue): \(error.localizedDescription); skipping (no template fallback)",
                level: .warning
            )
            return nil
        }
    ...
    private func fetchRecentMessages(limit: Int) async -> [ChatMessageSnapshot] {
        do {
            ...
            await logger.log(
                "Failed to fetch recent messages for health LLM: \(error.localizedDescription)",
                level: .error
            )
            return []
        }
...
enum HealthLLMError: Error, LocalizedError {
    case timeout
    case emptyResponse
    ...
    var errorDescription: String? {
        ...
        case .emptyResponse: return "LLM returned empty content"
        }
    }
}
