...
> 💜 如果你觉得 echo 有意思，欢迎在 [爱发电](https://afdian.com/a/rkny6) 投喂一点 token 钱，感谢支持独立开发。

---

## ⚠️ 免责声明（请务必阅读）

本项目基于 [MIT License](LICENSE) 开源，License 中已包含“按原样提供、不作任何担保”的标准条款。除此之外，结合本项目的具体形态，请你在使用前额外知悉以下几点：

- **个人早期项目，非商业产品**：echo 由个人独立开发，不隶属于任何公司，尚未上架 App Store，当前处于快速迭代阶段。代码可能存在未知 Bug，接口与数据结构可能随时变化，**请自行做好重要数据的备份**。因使用本项目（包括但不限于数据丢失、行为异常）造成的任何直接或间接损失，作者不承担责任。
- **不是专业医疗或心理健康建议**：echo 的"主动关怀""情绪陪伴"等功能基于规则判断与 AI 生成内容，**不能替代专业心理咨询、医疗诊断或紧急干预**。如果你或你身边的人正在经历心理危机、有自伤/自杀念头，请立即联系当地紧急服务或专业心理援助热线寻求帮助，不要依赖本 App 作为唯一支持渠道。
- **第三方服务费用自理**：如果你在设置中配置了云端 LLM、云端图像识别等第三方服务的 API Key，由此产生的调用费用发生在**你与该服务商之间**，与本项目及作者无关。请自行关注用量、设置额度上限，避免非预期扣费。
- **未成年人使用需监护人知情**：本项目未做年龄校验机制。如果监护人允许未成年人使用，请提前了解 echo 会处理的数据类型（尤其是健康数据与聊天内容，详见 [PRIVACY.md](PRIVACY.md)）与陪伴类功能的性质，并自行判断是否适合、如何监督使用。
...
在设置中开启 **用 AI 判断主动关心的时机** 后，模型会结合沉默时长、最近对话、关系记忆和当前时间，判断是否值得主动发消息，并生成候选内容。这是一个**脆弱且测试覆盖有限的 Beta 功能**：它依赖你配置的 LLM 服务，可能因网络、模型格式错误、超时或服务配置问题而跳过发送，也不保证判断始终符合预期。

该功能默认关闭，只改变主动关心的软判断；聊天空闲、冷却期、每日上限，以及晚间关心的既有时间窗口等硬性限制仍然生效。开启后，相关的记忆摘要、最近对话和时间信息可能会发送给你配置的 LLM 服务商；详见 [PRIVACY.md](PRIVACY.md)。
...
- 直接从 Xcode 点击 Run 覆盖安装时，只要 Bundle Identifier 不变，通常会保留聊天记录和其他本地数据
- 删除手机上的 App 会删除本地数据；echo 没有云端同步，重新安装后无法自动恢复聊天记录
